...
(* Take text from console and determine whether it 
*  conforms to the GCL grammar.
*)
let rec check input =
    try
    let e = parse input
...
let test1 = "ok" = (check "if  x != x -> x := x
[] x > x -> y:=z; do x = x -> z:=q od fi")
...
do x>0 -> y:=x*y;
          x:=x-1
od"
(*the last test is failing and we are aware of that, 
 *we will improve in following weeks. we ran out of 
 time.
*)
let print = function
